...
unacquainted with MLTT or explicit substitutions, so we summarize the salient points
...
a universe~$\cU$ is not inhabited by types directly,
but rather by the \emph{codes} of types:
$\codety{T}$ encodes type~$T$ (rule \ruleref{tm/c}),
and $\El{t}$ decodes~$t$ (rule \ruleref{ty/el}).
MLTT has an infinite hierarchy of universes;
we omit universe levels in the presentation for conciseness.
\rA{Why is W(t) a term and not a type?}\EDJreply{changed, please review.}\YZreply{I think the reviewer is wondering what is gained by encoding and decoding types, compared to types directly inhabiting universes.}
...
we can construct a term $\Wsup{\wsig}{t_1}{t_2}$ of the W-type. $\wcode{\wsig}$ gives the code of the
W-type (\ruleref{tm/w}).
...
We illustrate one concrete example of a "tm" with only two constructors, 
also as a review for how W-type encodes a given inductive type. The key insight 
is for a pair $(\wsigproj{i}{1}{\wsig}, \wsigproj{i}{2}{\wsig})$, the non-recursive
part of the given constructor stores in $\wsigproj{i}{1}{\wsig}$ and the ``arity'' of the 
recursive part stores in $\wsigproj{i}{2}{\wsig}$. For example, $"app" : ("tm" \to "tm" \to "tm")$ 
...
signature of our "tm" and showcase the meta-theoretic encoding of "unit" and "app~unit~unit". $"elim-"\bbzero$ indicates the elimination of the bottom type $\bbzero$ in the context. The key expressiveness of W-types comes from the ``arity'', as it encodes the case when recursive structure stores as the output of a function, mirroring the strict-positivity of Inductive type.  


\begin{figure}
  \begin{tabular}{
    >{\columncolor[HTML]{C0C0C0}}l lcl}
    \hline
    \multicolumn{1}{|l}{\cellcolor[HTML]{C0C0C0}surface syntax} &
      \multicolumn{1}{l|}{Equivalent Type} &
      \multicolumn{1}{l}{} &
      \multicolumn{1}{l|}{} \\
    \multicolumn{1}{|l}{\cellcolor[HTML]{C0C0C0}\lsti!| unit : tm!} &
      \multicolumn{1}{l|}{$\bbone \times (\bbzero \to tm) \to tm$} &
      $\tau'$ &
      \multicolumn{1}{l|}{$\coloneqq\WSigAdd{\WSigEmp}{\bbone}{\bbzero}$} \\
    \multicolumn{1}{|l}{\cellcolor[HTML]{C0C0C0}\lsti!| app  : tm -> tm -> tm!} &
      \multicolumn{1}{l|}{$\bbone \times (\bbtwo \to tm) \to tm$} &
      $\tau$ &
      \multicolumn{1}{l|}{\cellcolor[HTML]{FFFFC7}$\coloneqq\WSigAdd{\tau'}{\bbone}{\bbtwo}$} \\ \hline
    \multicolumn{1}{c}{\cellcolor[HTML]{C0C0C0}\lsti!unit!} &
      $\coloneqq \Wsup{\tau}{\unit}{"elim-"\bbzero}$ &
      \cellcolor[HTML]{C0C0C0}\lsti!app unit unit! &
      $\coloneqq\Wsup{\tau}{\unit}{"unit"}$
    \end{tabular}
\end{figure}
