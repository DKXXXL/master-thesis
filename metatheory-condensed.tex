...
For accessibility, we present MLTT using named variables and meta-level substitution,
however we emphasize that in the meta-theoretic reasoning carried out in the appendix, 
we will shift to debruijn indices and explicit substitution because it leads to clearer proof details.
We summarize the salient points before delving into the technical details for audience
unacquainted with MLTT.
...
\cref{fig:mltt-selected} presents the syntax and selected typing rules of (predicative) MLTT
(and \cref{fig:fmltt-selected} \TT). We formulate in an intrinsic style---every appearing syntax piece
is well-typed in prior, and the premise judgement required for well-typed might be omitted for conciseness. 
For example, the well-typed context $\goodCtx{\Gm}{}$ required by \ruleref{n/tm/lam} is omitted.  
We also use square bracket to indicate substitution. 
For each derivation (of each judgement) we introduce judgemental equality indicating convertibility 
in MLTT.  
We have dependent function and dependent tuple type equipped by standard dependent type theory.
...
%In dependent type system, types are treated as terms, and should have a type.
We use Tarski-style universes \cite{hofmann1997syntax}.
Unlike Russell-style universes, they
are not inhabited by types directly but rather by the \emph{codes} of types
and arguably behave better \cite{luo2012notes}.
$\codety{T}$ encodes type~$T$ (rule \ruleref{n/tm/c}) and thus inhabits~$\cU$.
$\El{t}$ decodes~$t$ (rule \ruleref{n/ty/el}).
...
Our MLTT has an infinite hierarchy of universes;
...
composed of a list (\ruleref{wsig/empty} and \ruleref{wsig/add}) of $n$ pairs of 
types $\{(\wsigproj{i}{1}{\wsig}, \wsigproj{i}{2}{\wsig})\}_i$,
modeling an inductive type of $n$ constructors. 
%
Rule~\ruleref{tm/wsup} states that the $i$-th pair $\goodType{\Gm}{\wsigproj{i}{1}{\wsig}}{}$ and 
...
$\goodTerm{\Gm,{\wsigproj{i}{2}{\wsig}}[t_1/x]}{t_2}{\El{\wcode{\wsig}}}$,
we can construct a term $\Wsup{i}{\wsig}{t_1}{a.t_2}$ of the W-type. $\wcode{\wsig}$ gives the code of the
W-type (\ruleref{tm/w}).
...
In rule \ruleref{l/add}, the second premise $\goodTerm{\Gm, a : A}{t}{T}$ is
...
the premise $\goodTerm{\Gm, a : A}{t}{T}$ makes clear that
...
The definitive version containing all the rules in \TT is stated in the appendix 
