...
formation rules that enable the further binding of W-types~\cite{martin1984intuitionistic} in linkages.
...
\begin{align*}
  ...
  {\Sigr{\Gamma}{(n+1)}} &= 
    \Sigma~(\Sigr{\Gamma}{n})
          ~(\Sigma~\cU
                  ~(\Sigma~(\Pi(\cC ("var"_0[p^1]))(El~"var"_0[p^1]))~\cU)) \\
  \Sigr{\Gamma}{0} &= \top \\
  ...
  \denotesT{\goodType{\Gamma}{\cC \sigma}{}} &= 
    \Sigma~\denotesT{\cC ("pjl"~\sigma)}~(El \ ("app"~("pjr"^3~\sigma))[(p^1, "app"~("pjl"~("pjr"^2~t)))]) \\
...
\end{align*}
\begin{align*}
  \denotesT{\goodInh{\Gamma}{\_}{\sigma_1}{\sigma_2}} &= \goodTerm{\Gamma, \cL\sigma_1}{\_}{\cL\sigma_2[p^1]} \\ \text{ and thus } \denotesT{\goodInh{\Gamma}{h}{\sigma_1}{\sigma_2}} &\iff  \goodTerm{\Gamma, \cL\sigma_1}{h}{\cL\sigma_2[p^1]} \\
  \denotesT{\goodInh{\Gamma}{"inhinh"~h~T~\uparrow^s}{(\nu^+ \  \sigma_1 \  T)}{(\nu^+ \  \sigma_2 \  T[(p^1, \uparrow^s)])}} &= \mu^+~(h[(p^1,p_1\mu~"var"_0)])\\ &\quad \quad \{f_2[p^1]\}~(p_2\mu'~"var"_0)[(p^1, \uparrow^s[{p^1}^{\uparrow}])] \\
\end{align*}
