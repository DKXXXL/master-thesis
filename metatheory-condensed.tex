...
Adding new types into signature \ruleref{Sig Add} requires a \textit{sealing judgement} $\goodSeal{\Gamma}{f}{\sigma}{A}$, which is an alias of a term judgement. Intuitively, sealing can abstract any given type by indicating a morphism to the interface. Sealing is a convenient device when programming using meta-theory, \textit{to indicate whether each field is overridable, non-overridable (thus definitionally exposed) or extensible}. For example, if a field is overridable, we abstract that to its type; if a field has exposed its definition, we abstract it into the singleton type containing it instead. 

However, there is subtle difference compared to the plugin implementation---$(\mu^+ ~o~\{f\}~t)$ reads as \textit{``$t$ is a new well-typed field when we abstract the \textbf{prior} fields using $f$''}. In other words, every field in meta-theory has a right to choose how they perceive and abstract the prior fields. While in the plugin, "Overridable" will decide how the current defining field is percived by the following fields.
...
Inheritance judgement can be further enriched with inheriting operation, overriding operation, extending operation and etc. These operations correspond to inherit, override and extend command inside "Family" in our plugin implementation.
...
% Focus on FRecursion and FInduction 
...
This above elaborated "hiding" syntax directly corresponding to the semantic of sealing---when definition is exposed, we expose a term by using singleton type, which will provide us a judgemental equality to the definition.