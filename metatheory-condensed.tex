...
Definitional equalities (for each judgement) have the form
\goodTerm{\Gm}{t_1 \equiv t_2}{T},\goodType{\Gm}{T_1 \equiv T_2}{},\goodCtx{\Gm_1 \equiv \Gm_2}{} ..., etc. We consider our syntax to be quotiented by these definition equalities, and thus each syntax piece is actually one (equivalent) class of syntax. Doing such quotient gives us convertibility for free---when we have definitionally equal $A \equiv B$, thanks to the quotient, the derivation of $t : A$ is the same as the derivation of $t : B$. \YZ{should there be a convertibility rule?}\EDJreply{in this style, the quotient by judgemental equality is applied, so A is equal to B even in meta-logic, which means t : A is the same as t : B. Maybe this is something require emphasis? Because it is usually handled by LF (like QIIT) but now we are using bare hands so.... maybe we need to emphasize this}
