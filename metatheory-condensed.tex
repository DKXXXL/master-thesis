...
Definitional equalities have the form
\goodTerm{\Gm}{t_1 \equiv t_2}{T} and \goodType{\Gm}{T_1 \equiv T_2}{}.\YZ{should there be a convertibility rule?}\EDJreply{in this style, the quotient by judgemental equality is applied, so A is equal to B even in meta-logic, which means t : A is the same as t : B. Maybe this is something require emphasis? Because it is usually handled by LF (like QIIT) but now we are using bare hands so.... maybe we need to emphasize this}
