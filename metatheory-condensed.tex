...
In the plugin implementation, "Family"s are second class objects like Coq's "Module". What's more, a "Family", with or without "extends" argument can be considered as a standalone piece of code trying to \textit{inherit} an existent (but maybe empty) family. Thus we model them using \textit{inheritance judgement} instead of considering them as first class terms. 




In this way, we will consider the program in our metatheory similar to those vanilla MLTT program, except there are meta variables denoting inheirtance judgement, of which the only usage is to apply to linkages. We don't yet consider non-trivial equality between inheritance judgements.
