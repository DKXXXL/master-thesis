...
projecting it (\ruleref{l/proj2}) and then including it.
...
$\goodTerm{A_i}{t_i}{T_i}$.
%
Here, we feel free to use a notation with explicit names.
We name the context as $\mathit{self}$ (cf.~\cref{fig:stlc-compiled})
and feel free to write its type, which is a dependent tuple type, as a
dependent record type with explicit labels.
%
...
%
Other fields automatically generated by the plugin, such as partial
recursors and various propositional equalities, can also be modeled in
this way by invoking $\texttt{Wrec}$.
...
responsible for hiding W-type signatures in~$\lsig$.
Here, $\TyTkg{\lsig}$ is a dependent tuple type (rule \ruleref{tyeq/pk/add})
packaging the types of all fields preceding the current field~$t$,
and $\seal$ is the term turning the tuple $\Tkg{\lkg}$ of type
$\TyTkg{\lsig}$ (rule \ruleref{tmeq/pk/add}) into a new term of type $A$
that hides W-type signatures behind $\cU$, if necessary.
When no hiding is needed, $\seal$ is $\var{0}$ (the identity) and $A$ is
$\TyTkg{\lsig}$.

When a family is closed and a complete linkage~$\lkg$ is formed, fields
of the family can be accessed by projecting them out of the tuple
$\Tkg{\lkg}$, as the last line of \cref{fig:stlc-linkage-typing} suggests.
...
\TT builds \emph{linkage transformers}, whose well-formedness judgments
have form $\goodInh{\Gm}{h}{\lsig_1}{\lsig_2}$.
Linkage transformers are second-class; the only way to use them is to
apply them to linkages to form new linkages (\ruleref{tm/inh}).
%
Derived families can be modeled as linkage transformers.
%
Linkage transformers are inductively constructed from several introduction
rules.
\cref{fig:typing-selected} shows the beta rule of an exemplary transformer,
\ruleref{lxeq/ov/beta}.
It can read as follows: given a linkage of form $\LkgAdd{\lkg}{t_1}$,
applying the transformer $\InhOv{h}{t_2}$ overrides the linkage's last
field~$t_1$ with~$t_2$.
$\InhOv{\InhId}{\wcode{\wsig'_{\texttt{tm}}}}$, for instance, can be used
in \cref{fig:stlc-linkage-typing} to form a derived family that
...
The definitive version is formalized in a logical framework with
quotient inductive-inductive types~\cite{nordvall2010inductive,altkap2016},
available as a supplemental file in Agda syntax.
%
We also typeset a more complete set of rules than
\cref{fig:mltt-typing-selected,fig:fmltt-typing-selected} allow
and present it in a supplemental appendix.
%
Another supplemental appendix gives a larger-scale example than
\cref{fig:stlc-linkage-typing} showing how families are modeled and
...
\newcolumntype{C}{>{$}c<{$}}
\newcolumntype{R}{>{$}r<{$}}
\begin{tabular}{@{}lR@{\ }C@{\ }LR@{\ \,}C@{\,\ }L@{}}
\lsti!Family STLC.!
& \lkg_0 & \coloneqq & \LkgEmp
\\
\lsti!FInductive tm :=!
& \lkg_1 & \coloneqq & \LkgAdd{\lkg_0}{\dadada}
& \mathit{self} : []
& \vdash
& \wcode{\wsig_{\texttt{tm}}} : \TyS{\wcode{\wsig_{\texttt{tm}}}}
\\
\lsti!| tm_unit : tm | !\dadada
& \lkg_2 & \coloneqq & \LkgAdd{\lkg_1}{\dadada}
& \mathit{self} : [\texttt{tm}:\TyS{\wcode{\wsig_{\texttt{tm}}}}]
& \vdash
& \Wsup{\wsig_{\texttt{tm}}}{\top}{\bot} : \El{\mathit{self.}\texttt{tm}}
\\
\lsti!FRecursion subst !\dadada
\\
\lsti!Case tm_unit := !\dadada
& & &
& \mathit{self} : [\texttt{tm}:\cU, \texttt{tm\_unit}: \El{\texttt{tm}}, \dadada]
& \vdash
& \dadada : \CaseSig{\top}{\bot}{\texttt{id}\to\El{\textit{self.}\texttt{tm}}\to\El{\textit{self.}\texttt{tm}}}
\\
\lsti!End subst.!
& & &
& \mathit{self} : [\texttt{tm}:\TyS{\wcode{\wsig_{\texttt{tm}}}}, \texttt{tm\_unit}: \El{\texttt{tm}}, \dadada]
\\
\lsti!!
\end{tabular}
