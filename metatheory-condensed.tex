...
\item 
Linkages model families, so they are like tuples composed of the fields.
But there is a twist: they support late binding.
...
In particular, when no hiding is needed, $\seal$ is $\var{0}$ (the identity)\EDJ{$\var{0}$ is not identity (substitution), but the first variable, right? Or you meant to say it is identity ``sealing''?} and $A$ is
$\TyTkg{\lsig}$.
