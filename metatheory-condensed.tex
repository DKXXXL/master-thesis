...
Now, we will mainly focus on the newly introduced typing rules, and illustrate their connection to the plugin implementation. For those curious about the complete meta-theory formulation, we refer to our appendices or \citet{altkap2016, kaposi2017type} or maybe those not using QIIT \cite{coquand2018canonicity, sterling2019algebraic}. Our metatheory is formulated using QIIT in an extensional setting.\YZ{
...
But we can never have a term $x : \cL\{a : S(\bot), b : a \to \bot \} \vdash h_2 : \cL\{a : \cU, b : a \to \bot \}$, as we know $\cL\{a : S(\bot), b : a \to \bot \}$ is inhabited\YZ{How is this type inhabited? Through an identity seal?}\EDJreply{Yes. a = $\bot$, b is identity. $S(\bot)$ is a singleton type with only bottom type as element inside} and that leads to a close term of $\cL\{a : \cU, b : a \to \bot \}$ due to the nature of late-binding of $\cL\sigma$---once we override $a$ with $\top$ we will have inconsistency using $b$. This example illustrates the programming difference between the sigma type and linkage type.  
...
{\goodTerm{\Gamma}{"prjR"~j~o}{("RecSig1"~(\pi_1^j \tau)~(\pi_2^j \tau)~R)}}\YZ{Why is this pi_1 substitution needed?}\EDJreply{You are right, I think it is a typo.}
$$
$$
...
\quad \goodTerm{\Gamma}{w}{El~("W"~T)}\YZ{Is the 1st premise implied by the 2nd, and the 3rd implied by the 4th?}\EDJreply{Yes.}
}
{\goodTerm{\Gamma}{"Wrec"~r~w}{R}}\YZ{I suppose Rec-Constr is only for exhaustivity checking? If so, then would it be more intuitive to not include the w in the term? And I suppose the programmer will want to use sealing to hide its type, or otherwise it would fail to normalize for lack of a beta rule?}
$$
