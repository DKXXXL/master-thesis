...
in its type (rule \ruleref{n/tm/s})~\cite{aspinall1995singleton,stone2000}.
For each derivation (of each judgment) we introduce judgmental equality
(of the form \goodTerm{\Gm}{t_1 \equiv t_2}{T})
indicating convertibility.

We use Tarski-style universes \cite{hofmann1997syntax},
which, unlike Russell-style ones,
are not inhabited by types directly but rather by the \emph{codes} of types
and arguably behave better \cite{luo2012notes}.
Term $\codety{T}$ encodes type~$T$ (\ruleref{n/tm/c}) and thus inhabits universe~$\cU$.
Type $\El{t}$ decodes term~$t$ (\ruleref{n/ty/el}).
...
For conciseness, typing rules omit obvious premises required for well-formedness.
For example, \ruleref{n/tm/lam} implicitly requires that the context be well-typed
($\goodCtx{\Gm}{}$).
...
% yields terms valid in~$\Gm$ (\ruleref{n/tm/sub} and \ruleref{n/ty/sub}).
% %
% The two main forms of substitutions are weakening (\ruleref{n/sub/wk}) and extension (\ruleref{n/sub/ext}):
...
% then applies~$\gm$.
% For example, rule \ruleref{n/tm/snd} states that if $t$ is a dependent pair
% that has type $\TySigma{A}{B}$, then $\snd{t}$ has type
% $\sub{B}{\SubstExt{\SubstWeak{0}}{\fst{t}}}$,
% where $\SubstId$ is the identity substitution (\ruleref{n/sub/id}).
...
%
Our enhanced W-type is decided by a W-type signature $\goodWSig{\Gm}{\wsig}{n}$ 
composed of a list (\ruleref{n/wsig/empty} and \ruleref{n/wsig/add}) of $n$ pairs of 
types $\{(\wsigproj{i}{1}{\wsig}, \wsigproj{i}{2}{\wsig})\}_i$,
modeling an inductive type of $n$ constructors. 
%
Rule~\ruleref{n/tm/wsup} states that the $i$-th pair $\goodType{\Gm}{\wsigproj{i}{1}{\wsig}}{}$ and 
$\goodType{\Gm,\wsigproj{i}{1}{\wsig}}{\wsigproj{i}{2}{\wsig}}{}$ models the $i$-th constructor---given 
two arguments $\goodTerm{\Gm}{t_1}{\wsigproj{i}{1}{\wsig}}$ and
$\goodTerm{\Gm, \mathit{self} : \nsub{\wsigproj{i}{2}{\wsig}}{x}{t_1}}{t_2}{\El{\wcode{\wsig}}}$,
we can construct a term $\Wsup{i}{\wsig}{t_1}{\mathit{self}.t_2}$ of the W-type. $\wcode{\wsig}$ gives the code of the
W-type (\ruleref{n/tm/w}).
...
case handlers for the $n$ constructors in~$\wsig$ (\ruleref{n/tm/wrec}).
Each case handler has a type of form $\CaseSig{A}{B}{T}$, where
$T$ is the motive of the recursion (\ruleref{n/tyeq/casety});
...
Rules \ruleref{n/tm/wsup} and \ruleref{n/tm/wrec} require access to~$\wsig$:
...
Well-formedness judgments of \emph{linkage signatures} have the form
$\goodSig{\Gm}{\lsig}{n}$.
As rules \ruleref{n/lsig/empty} and \ruleref{n/lsig/add} show, a linkage
signature is a list of $n$~types.
%
Well-formedness judgments of \emph{linkages} have the form $\goodTerm{\Gm}{\lkg}{\TyLkg{\lsig}}$,
where $\TyLkg{\lsig}$ is a type formed by~$\lsig$.
As rules \ruleref{n/l/empty} and \ruleref{n/l/add} show, a linkage is a list
of $n$~terms, each representing a field of the family modeled by the linkage.

In rule \ruleref{n/l/add}, the second premise $\goodTerm{\Gm, \mathit{self} : A}{t}{T}$ is
...
Crucially, %unlike the second field in a dependent pair (\ruleref{n/tm/pair}),
the premise $\goodTerm{\Gm, \mathit{self} : A}{t}{T}$ makes clear that
the typing of~$t$ in \ruleref{n/l/add} is \emph{universally} quantified---%
rather than \emph{existentially} quantified as is in \ruleref{n/tm/pair}---%
...
The type~$A$ in \ruleref{n/l/add} and \ruleref{n/lsig/add},
...
should see the concrete signature~$\wsig$, as rules \ruleref{n/tm/wsup}
and \ruleref{n/tm/wrec} stipulate.
...
as required by rule \ruleref{n/tm/wsup}.
...
in rules \ruleref{n/l/add} and \ruleref{n/lsig/add} is
responsible for hiding W-type signatures.
Here, $\TyTkg{\lsig}$ is a dependent tuple type (rule \ruleref{n/tyeq/pk/add})
...
$\TyTkg{\lsig}$ (rule \ruleref{n/tmeq/pk/add}) into a new tuple of type~$A$
...
type~$\TyLkg{\lsig_2}$ (\ruleref{n/tm/inh}).
...
% conversion rules (e.g., \ruleref{n/tmeq/sub/id}).
...
\else a supplemental appendix.
\fi
using debruijn indices and explicit substitution.
