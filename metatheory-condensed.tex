...
\ruleref{lkg/proj} indicates how linkage captures the idea of \textit{code reuse}, as $\lkgproj{2}{-}$ will lead to the boxed term that is parametric on the surrounding linkage $\lsigprojT{\lsig}$ . To code reuse, we simply project out a field from the parent and add back to the children family.
\newpage 
...
Each hiding comment corresponds to the textual-aligned sealing derivation on RHS. For eligibility, curly brackets indiate a signature (omitting sealing at type level for readability) and square bracket indicate a nested sigma type, corresponding to sealing judgement $\goodSeal{}{s}{\sigma}{A}$.
