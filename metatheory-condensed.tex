...
When a family is completed (e.g., \lsti!End!~\lsti!STLC!),
...
constructed from the introduction forms $\InhId$, $\InhExt{h}{t}$, $\InhOv{h}{t}$, etc.
...
the space allows and present it in a supplemental appendix.
%
...
\begin{figure}[!htb]
  ...
  ↑s₃ : Tm (⋅, [ty : 𝕌; ...; tm_true : tm; ..]) [ty : 𝕌; ...] (* the sealing before extension *)
  ii₃₀₀ = inh-id 
        : Inh (⋅, [ty : 𝕌; ...; tm_true : tm; ..])
              (RecSig τ₂  tm)[p₀, ↑s₃]
              (RecSig τ₂  tm)[p₀, ↑s₃]
  ii₃₀₁ = inh-extend (inh-extend ii₃₀₀ {p₁} .. ) {p₁} ..
        : Inh (⋅, [ty : 𝕌; tm :𝕌; tvar : nat → tm; ...; tm_true : tm]) (RecSig τ₂   tm)[p₀, ⇡s] (RecSig τ₂'  tm)
  i₃₁ = inh+inh i₂ ↑s₃ ii₃₀₁ : 
      Inh ⋅ {ty : 𝕊(W τ₁); tm : 𝕊(W τ₂); subst' : 𝕃(RecSig τ₂ tm)} {ty : 𝕊(W τ₁'); tm : 𝕊(W τ₂'); subst' : 𝕃(RecSig τ₂' tm)}

  (* s₃' : {ty : 𝕎(τ₁'); tm : 𝕎(τ₂'); subst' : 𝕃(RecSig τ₂' tm) }
          | [ty : 𝕌; ... ; tm : 𝕎(τ₂'); subst' : 𝕃(RecSig τ₂' tm)] *)
  i₃ = inh-ov i₃₁ ..
    : Inh ⋅ {ty : 𝕎(τ₁); tm : 𝕎(τ₂); subst' : 𝕃(RecSig τ₂ tm); subst : tm → nat → tm → tm}  {ty : 𝕎(τ₁'); tm : 𝕎(τ₂'); subst' : 𝕃(RecSig τ₂' tm); subst : subst : tm → nat → tm → tm}
  (* recall s₄ : {ty : 𝕎(τ₁); ... subst : tm → nat → tm → tm } 
      | [ty : 𝕌; ... ; tm : 𝕌; ... ; subst' : 𝕃(RecSig τ₂ tm); subst : tm → nat → tm → tm] *)
  s₄' : {ty : 𝕎(τ₁'); ... subst : tm → nat → tm → tm } 
        | [ty : 𝕌; ... ; tm : 𝕌; ... ; subst' : 𝕃(RecSig τ₂' tm); subst : tm → nat → tm → tm]
  ↑s₄ : Tm (⋅, [ty : 𝕌; ... ; tm : 𝕌; ... ; subst' : 𝕃(RecSig τ₂' tm); subst : tm → nat → tm → tm]) [ty : 𝕌; ... ; tm : 𝕌; ... ; subst' : 𝕃(RecSig τ₂ tm); subst : tm → nat → tm → tm]
  i₄ = inh-inh i₃ ↑s₄ :
    Inh ⋅ {ty : 𝕎(τ₁); ... subst : tm → nat → tm → tm; 𝕊(tm_var 0)} 
          {ty : 𝕎(τ₁'); ... subst : tm → nat → tm → tm; 𝕊(tm_var 0)} 
  \end{lstlisting}
