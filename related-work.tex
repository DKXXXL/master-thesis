There exist many approaches to modular mechanization or proof reuse,
...
having to manually prove additional well-formedness conditions on the algebras.
...
as \citet{liwei2022} observe, in that extending a composed
mechanization requires recompilation.
...
override and families as mixins, also facilitate extensibility and reuse
...
allowing the programmer to write code that has built-in hooks for
future extensions.
...
an idea found in the context of OO programming,
to the context of mechanized proofs.
Families in \Lang are second-class, in that they are not considered
terms by themselves.
Families as modeled in \TT (namely linkages) are first-class and thus more flexible.
The distinction corresponds to class-based vs.\ object-base family
polymorphism as summarized before~\cite{cdnw07-tribe}.

\citet{bac2006} address the expression problem in a functional language,
introducing a language mechanism that allows pattern-matching cases to
be added retroactively in accordance with extensions to variant types.
Our \lsti{FRecursion} and \lsti{FInduction} commands achieve a similar
functionality, with families bestowing additional organizational power.
