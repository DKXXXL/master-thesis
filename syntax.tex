...
\begin{figure}[!htb]
  ...
  \end{align*}
\caption{Judgements, as Agda types and as presented in this section.}\label{fig:vanillaMLTTjudgeQIIT}
\end{figure}
\begin{figure}[!htb]
...
% Judgements in QIIT
We formalize our language as a deep embedding in a type theory using QIIT types. Every judgement is a (dependent) QIIT \textit{type} and all derivations of judgement  are well-typed QIIT terms. (Otherwise how can they be encoded in Agda?) There are four standard judgement forms from (vanilla) MLTT,
...
Latex-wise, in \cref{fig:vanillaMLTTjudgewellformed}, we spell out these \textit{well-formedness rules} of each
judgement and their (pseudo-)Agda counterparts to make things more accessible. In (pseudo-)Agda, well-formedness rules are just well-typedness of (indexed) QIIT family---(the judgement)"Tm~$\Gamma$~$T$" is well-typed (well-formed) only when $\Gamma$ and $T$ are both well-typed (well-formed). We omitted a bunch for latex version. Of
...
using, there is always equality among the derivations but only those equalities between terms and types are emphasized due to their non-triviality.


% Example : how judgemental equality is used for the universe
We review the concept of ``universe'' and at the same time showing how equality is representing semantic. In dependent type setting, function now can return type, of which the type is ``type of type'', i.e., \textit{the universe} $\cU$. We follow the style of \textit{name=ty/universe à la Tarski}~\cite{hofmann1997syntax} and
...
$\goodType{\Gamma}{El~T}{}$, and vice-versa.\YZ{Why is it wanted to distinguish types from their codes?}\EDJreply{Because in formalization, for a judgement $G |- t : T$, code can only be placed at $t$ and type can only be placed at $T$. In Russel style, they don't do such distinction. But for the sake of formalization, we will distinguish code and types.}
...
Thus (1) \ruleref{subst/ext} looks like concatenating one
...
the context $\Delta$ so that we can use \ruleref{ctx/ext} to
...
Consecutive substitutions can be composed using \ruleref{subst/comp}, because
...
\begin{figure}[!htb]
  \begin{alignat*}{3}
    ...
    \text{Terms, including Seal}   \quad & t, t_1, t_2, f    &&= ...~|~"W"~T~|~"Wsup"~T~t_1~t_2~|~\mu\cdot~|~\mu^+~t~\{f\}~T~|~p_1\mu~t~|~p_2\mu~t~|~\cCt~t~|\\ & &&"prjR"~j~o~|~"Wrec"~t_1~t_2~|~"inh"~h~t\\
    \text{Inheritance Structure} \quad & h, h' &&= "inhid"~|~"inhov"~h~t~|~"inhext"~h~t~|~"inhinh"~h~|~"inh"~h~|~"inhnest"~h~h'
  \end{alignat*}\YZ{ What is 'W T'? Do you mean 'c T'?  }\YZreply{Why is Seal not merged into Terms?}\EDJreply{You can see \ruleref{Ind-Type}. For $T : \bW\tau$, $W~T : \cU$, for $\goodType{\Gamma}{T}{}$, $c~T : \cU$ }\EDJreply{Seal can easily mergedd into terms. But seal has a new judgement. Do you want to merge them? }
  \YZ{The grammar suggests linkages are first-class terms. But aren't linkages second-class citizens?}\EDJreply{Linkages are first-class citizens in metatheory. Linkages are second class citizens in Coq plugin. Inheritance judgement is second class in plugin and metatheory. }
...
&\sigma~:~"Sig"~\Gamma~n&&\mathboxtext{\goodSig{\Gamma}{\sigma}{n}}  &&\tau~:~"WSig"~\Gamma~n&&\mathboxtext{\goodWSig{\Gamma}{\tau}{n}} \\ &h~:~"Inh"~\Gamma~\sigma_1~\sigma_2&&\mathboxtext{\goodInh{\Gamma}{h}{\sigma}{\tau}} && f~:~"Seal"~\Gamma~\sigma~T&&\mathboxtext{\goodSeal{\Gamma}{f}{\sigma}{A}}
  \end{align*}
\caption{Extended Judgements, as Agda types}
\end{figure}
\begin{figure}[!htb]
  \begin{minipage}[b]{0.3\linewidth}
      $$
      ...
      $$\YZ{Doesn't the left column show well-formedness rules for ONLY TWO of the four new judgments? Besides, are these rules used anywhere in the following? Seems a bit unusual to have well-formedness rules for judgments per se.}\EDJreply{Yes. Right column show all the well-formedness rule for all the judgements. That is unusual because it is a direct translation from QIIT. }
  \end{minipage}
  \begin{minipage}[b]{0.6\linewidth}
    \begin{minted}[]{agda}
      data Sig  : Con → ℕ → Set
...
The core language builds on MLTT and adds four new kinds of judgements.
...
{\goodTerm{\Gm}{\Wsup{j}{T}{a}{b}}{\El{(\wcode{\wsig})}} }
$$
...
\\ \goodType{\Gm, \lsigprojT {\lsig}}{\lsigproj{2}{(\LSigAdd {\lsig} {\seal}  {T})} \equiv T}{}
}
$$
...
\Rule[name=Lnkg Proj]
{\goodTerm{\Gm}{o}{\cL\lsig}}
{\goodTerm{\Gm}{\lkgproj{1} {o}}{\cL (\lsigproj{1} {\lsig})}
...
\quad  \goodSeal{\Gm}{f}{\lsig}{A} 
 \quad \goodTerm{\Gm, A}{t}{T}}
{\goodTerm{\Gm}{\lkgproj{1} {(\LkgAdd {o} {t})} \equiv o}{\cL \lsig}
...
\caption{Sealing Judgement, and its Agda Representation (or What do we mean when we say one judgement is a shorthand for the other)}
...
%}\YZreply{Yes, sf' is confusing to me.}\EDJreply{sf' notation removed. Please check.}
Note that, given $\goodSeal{\Gamma}{s}{\sigma}{A} = \goodTerm{\Gamma, \cC\sigma}{s}{A}$, notation "sf"
...
% \YZ{Why do Lnkg Add and Sig Add leave A unspecified? I thought this A corresponds to 'self'.}\EDJreply{You are asking where is the function \cC \{a : \bW sigma ; ..\} to \cC\{a : \cU ; ..\}, which by definition is requesting (Sigma (a : \bW sigma ; ..)) to (Sigma (a : \cU ; ..)). \\
% This is something doable in vanilla MLTT, with the help of transforming \bW sigma to \cU, by rule `Ind-Type'. And this function's type should be directly the sealing type.}\YZreply{
%   My confusion is this: it seems that in Lnkg Add and Sig Add, A could well be the top type and f a value of the top type. In this sense, A seems unspecified.
%   Or maybe in the metatheory, it is the programmer's responsibility to find this A?
% }\EDJreply{You are absolutely right. In metatheory it is programmer's responsibility to specify the A they want. If they set A to top, they can do that but it will be hard for them to achieve meaningful program. \\
% Also the `f' is specified together with `A' by the user as well. \\
% A bit off-topic : this is not the only place that such specification is postponed to the user (in metatheory) another place is in inheritance when specifying upcasting function.}
...
\textit{recursors}.\YZ{Metatheory should mention how computational axioms about case handlers, partial recursors, and axioms over partial recursors are dealt with in the metatheory.}  
...
%\YZ{Is sealing really hiding these fields (that use Wsup and Wrec), or is it that the concrete definition of an inductive type is not hidden for these fields so that they can assume knowledge about the inductive type's signature?}\EDJreply{More rigourously speaking, sealing is only hiding the $\tau$ appearing in $\bW \tau$, it never tries to hide (or make absence of) any field.
%}\YZreply{The sentence feels confusing because "these fields" are not sealed; rather, as you explain below, there's no need to seal anything when, say, doing exhaustiveness checking using Wrec.}\EDJreply{Oh I see. The sentence here I say "fields and the inductive type" are sealed together because the sealing function will be between big sigma (Sig T:$\bW \sigma$, c1 : El (W T) -> El (W T)) and (Sig T:$\cU$, c1 : El T -> El T). That is the reason I say ``sealed together'' here. The (constructor) fields (e.g. c1) are not literally hidden, but their type are changed *together* with the former type T, because of type-dependency. }
After compilation
...
% With sealing on hand, our metatheory is directly corresponding to our Coq's implementation. 

Partial recursor and computational axiom is modelled like any other recursors and theorems.\footnote{This is partially because the partial recursor is provable in our plugin.} Recall that the sealed context is corresponding to a module type. Partial recursor is constructed directly using \ruleref{Rec-Constr} like any other recursors in our metatheory. With trivial sealing, and with concrete inductive type and partial recursor exposed in the context, we will have judgemental equality for beta-equality of the partial recursor, to prove the computational axiom. The only difference is that, partial recursor doesn't support user extension in plugin implementation but our encoding supports that. 
...
$\goodSeal{\Gamma}{f_2}{\sigma_2}{A_2}$ in the premises of rule
when needed.
...
{\goodInh{\Gm}{\InhNest{h}{i}}{(\LSigAdd {\sigma_1} {\seal_1}  {\cL\tau_1})}{(\LSigAdd   {\sigma_2} {\seal_2} {\cL\tau_2)}}}
\quad
$$
...
Similar to plugin implementation and \ruleref{Lnkg Add}, we resolve
...
The deduction rules of the inheritance judgement is very close to how Family is
...
Override "inhov" is even more special because in mundane OO programming
...
Next, we introduce nested inheritance "inhnest" for dealing with nested families. The rule is mostly direct and we need ``upcasting'' again when dealing with nested family in different but inheriting context.  We also show how to use an inheritance judgement and their derivation to construct children family and how the beta rule is defined. We omit most of the other substitution laws here.
