...
\quad 
\Rule[]{}{\goodTerm{\Gamma}{\star}{\bW \tau}}\YZ{What is star? Is it just tau??}\EDJreply{star is a convention indicating this $\bW\tau$ is a singleton type, in other words, there is an eta(uniqueness) rule for arbitrary $a : \bW$,$a = \star$. But here I just want to show the constructor instead of the uniqueness rule. TBH I don't know what other good name (other than $\star$) can be used in  constructor/intro rule }
\quad
\Rule[name=Ind-Term]
{\goodTerm{\Gamma}{T}{\bW \tau}
...
{\goodInh{\Gamma}{"inhinh" \ h}{(\nu^+ \  \sigma_1 \  T)}{(\nu^+ \  \sigma_2 \  T[(\pi_1, \uparrow^s)])}}\YZ{Should the term be 'inhinh h t' instead of 'inhinh h', where t is the field being inherited??}\EDJreply{$t$ is here the term in the field, not field name(we don't have name in metatheory) or type (type is T, it can be included). So no. Because the point of 'inhinh' in inheritance judgement is that when writing down inheritance, the children family doesn't have to include the original term. }
$$
$$
\Rule[name=Inh]
{\goodInh{\Gamma}{h}{\sigma_1}{\sigma_2}
