...
to its closedness to categorical semantic. We will go through these details one-by-one.

\underline{\textit{\textbf{Judgements and Universe.}}}
% Judgements in QIIT
We formalize our language as a deep embedding in a type theory with QIIT types. Every judgement is a (dependent) QIIT \textit{type} and all judgement derivations are well-typed QIIT terms. (Otherwise how can they be encoded in Agda?) There are four standard judgement forms from (vanilla) MLTT,
...
spell out that the rule \ruleref{Type Universe} (that is stating a universe is a proper type) requires a well-formed
...
distinguish types from their \textit{codes or names} as showin in \ruleref{Codes/Get Type}: given a \textit{code} (actually a term) $\goodTerm{\Gamma}{T}{\cU}$ we can retrieve the corresponding type
$\goodType{\Gamma}{El~T}{}$, and vice-versa (given a  
...
type, with the following \textit{two (omitted) equations representing their semantic} $"c" (El~T) \equiv T$ and $El
("c"~T) \equiv T$---notice how the two sides of each equality is term of the same type thus well-typed. More concretely, 
...
first-class citizens, and lambdas can wrap (almost) every parts and can return
