...
types are the ones sealing is responsible for hiding.\YZ{Is sealing really hiding these fields (that use Wsup and Wrec), or is it that the concrete definition of an inductive type is not hidden for these fields so that they can assume knowledge about the inductive type's signature?}\EDJreply{More rigourously speaking, sealing is only hiding the $\tau$ appearing in $\bW \tau$, it never tries to hide (or make absence of) any field. But for the people that are only looking at the interface/type, the people cannot if a field is a constructor (using Wsup) or a normal function. \\
The concrete definition of an inductive type (i.e.  $\bW \tau$) is definitely hidden and those inductive type will thus have type $\cU$. After the $\tau$ is hidden, the programmer is assuming knowledge of the inductive type.
