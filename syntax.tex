...
\\ \goodType{\Gamma}{p_1\nu' \ \sigma}{}
\\ \goodSeal{\Gamma}{p_f\nu \  \sigma}{p_1\nu \  \sigma}{p_1\nu' \sigma}
\\ \goodType{\Gamma, p_1\nu' \ \sigma}{p_2\nu \ \sigma}{}\YZ{Add beta or eta rules here to explain syntax term clearer}
}
$$
...
\\ \goodType{\Gamma, p_1\nu' \ \sigma}{p_2\nu \ (\nu^+ \ \sigma \ \{f\} \ T) \equiv T}{}\YZ{Add beta or eta rules here to explain syntax term clearer}\EDJreply{Done}
}
$$
...
$\goodTerm{\Gamma, \cC \sigma}{sf'~s}{A[\pi_1]}$.\YZ{sf' seems unnecessary, since sf' s ad s are the same?}
Notation "sf"
...
to be in the context of the abstraction $A$\YZ{Why do Lnkg Add and Sig Add leave A unspecified? I thought this A corresponds to 'self'.}
...
the plugin. Our plugin will deduce a sealed ``interface''\YZ{Does this 'interface' correspond to A?}
for a sealed family using a fixed set of rules, but here sealing in
metatheory can be arbitrary local morphism.\YZ{why?}
