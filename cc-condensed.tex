...
amounts to a normalization function for closed terms of the ground type\EDJ{Suggesting use "boolean type". Because natural number type can be also considered ground type in other context (but this theorem on boolean cannot ensure canonicity for nat). We also does specify what is ground type. }.
...
closed, ``reducible''\EDJ{I will add a it is my bad to introduce "reducible" without defining it first. But my intension is to give a feeling of the dynamic operational semantic for the reader (of course OS is not here in our setting)} terms of type~$T$.
