...
and its context \lsti{STLC\_ty\_Ctx}\EDJ{Why I see in the figure there are only two includes, i.e. only stlcty and stlctyctx}.
...
its constructors (e.g., \lsti{tm_abs}) \EDJ{I am not sure if it is better we emphasize in the following way :`` that declares a tm type, its constructors (e.g. tmabs) as a normal function,.. '' i.e. emphasize it is a normal function in the module type},
its partial recursor (\lsti{tm_prect_STLC}),
and the propositional equalities\EDJ{I propose to change this sentence to ``and the prop eq about computational behaviour on the partial recursor''. And thus in the other parts of the paper, we can use the word ``computational behaviour'' instead of prop eq. Because just saying prop eq seems a bit of lack of context/details.} on the partial recursor
(e.g., \lsti{tm_abs_eq_STLC}; \cref{sec:prec}).

Importantly,
\lsti{STLC\_tm} merely declares the existence of these names and their types;\EDJ{What does this sentence mean?}
...
its propositional equalities.\EDJ{Simiarly, here I think using ``computational behaviour'' is more accurate than using the word ``prop eq''}
