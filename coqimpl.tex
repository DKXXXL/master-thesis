...
example field "add".\YZ{This 'add' field is no longer there.} Now that we only expose the type of "add", we
...
inductive types?}---and engineering),\YZ{I do consider that your plugin supports extensible inductive types. It's just that they are implemented as a plugin, rather than in the Coq core.}
but we simulate extensible
...
To construct a recursor as in \ref{langdesign-req3}, we\YZ{A source of confusion in this section is that the reader often has to guess who performs the actions: is it the programmer or the Coq plugin?} create for each
