...
\subsection{Language Design}\YZ{I think in §3.1, you should use a figure to show how the actual code the programmer will write to solve the programming challenge outlined in Figure 1.}\EDJreply{Done.}
...
To resolve this, we provide \textit{family sealing} mechanism to simulate the sigma type. In a sealed family, every field will expose its definition inside the family but a family as a whole will be sealed into a ``relaxed'' type. Thus we can override one sealed family as a whole and change the internal implementation. 
