...
The \Lang implementation currently does not support Coq's full
facility for inductive types and fixpoints;
in particular, mutually inductive types and mutual fixpoints are not supported.
The plugin supports indexed inductive types but not parameterized ones, though
the latter can be encoded via the former.
Partial recursors are automatically generated only for non-indexed inductive types.
We believe that these limitations do not pose conceptual challenges and can be
...
However, there are also limitations we believe require more research effort or more thought from a language-design perspective, including but not limited to: 
\begin{itemize}
  ...
  \item The current recursion mechanism supported by "FRecursion" and "FInduction" only works at top level. Further research efforts are necessary to enable the appearance of recursion handlers at any location while facilitating extensibility of the handlers at said locations. Additionally, a suitable Coq tactic is required to accompany this functionality. \YZ{mention restricting induction to the top level}
