...
\mintinline{Coq}{(f : nat → nat; p : monotonic f)}\YZ{Is this semicolon syntax valid Coq syntax?}\EDJreply{No, this part is just pseudo-code for two consecutive fields. I have no good idea on what to use other than semicolon, apparently I cannot use comma.} because to provide
...
users to specify the compiled module type of a given family\YZ{I don't understand "specify the family/compiled module type"}\EDJreply{Sorry. I rephase it into "compiled module type of a given family".}, and thus we can have a
...
\mintinline{Coq}{(f : nat → nat, p : monotonic f)} together inside an
...
``Sealed'' families can actually simulate interfaces in certain sense\YZ{I was actually thinking sealed families simulate sigma types, no?}\EDJreply{You are absolutely right. I should add a "can" here. I can use sealed family to simulate interface is by creating a sigma type, but making the impleemntation of sigma type trivial (using place holder/using axiom as I specified below). Then once I want to instantiate with real implementation, I override the whole family -- as long as it has the same type as the sealed family, everything still works.}---we can assign each field of the sealed family with \mintinline{Coq}{Axiom non_implement : forall {T : Type}, T.}, and later instantiate them with concrete implementation by overriding this family.\YZ{I don't understand what it means to "assign each field with an axiom".}\EDJreply{It really means ``place holder'' in general programming language (Python's NonImplement()), defer to future. Because Coq's Axiom can be used to instantiate anything.}
