...
An inheritance data\YZ{what is 'inheritance data'?} will be annotated with the type for the input family
...
\mintinline{Coq}{(f : nat → nat; p : monotonic f)}\YZ{Is this semicolon syntax valid Coq syntax?} because to provide
...
users to specify the family/compiled module type\YZ{I don't understand "specify the family/compiled module type"}, and thus we can have a
...
``Sealed'' families actually simulate interfaces in certain sense\YZ{I was actually thinking sealed families simulate sigma types, no?}---we can assign each field of the sealed family with \mintinline{Coq}{Axiom non_implement : forall {T : Type}, T.}, and later instantiate them with concrete implementation by overriding this family.\YZ{I don't understand what it means to "assign each field with an axiom".}
