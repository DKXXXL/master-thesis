...
Inductive C : Set := ... | cᵢ : forall (x : Aᵢ), (Bᵢ x -> C) -> C | ...
    ...
    \begin{itemize}
      ...
      \begin{minted}[escapeinside=@@]{Coq}
prec : forall (R : Set), ..,  @\YZ{Can R be indexed?}\EDJreply{Yes. Discussed below. Basically constructing left inverse for simple inductive type, this weaker form is enough}@
...
\begin{minted}{Coq}
  pdelim : ∀ (P : D -> Set), .., 
  (rᵢ : (∀ (x : Aᵢ) (w : Bᵢ x -> D), (∀ (b : Bᵢ x), P (w b)) -> (P (dᵢ x w)))),  
  .., ∀ (d : D) -> option (P d)
\end{minted}
