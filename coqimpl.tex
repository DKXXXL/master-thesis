...
\begin{itemize}
\item Parameterized by enclosing family "self": explicit extensibility hooks
\item "self" includes only those fields in the context
\item Extensible inductive type $\Rightarrow$ axiomatize
\item Further-bindable \lsti{FRecursion} $\Rightarrow$ axiomatize
\item Non-overridable definitions $\Rightarrow$ expose definitional equality
\item Instantiate inductive types, partial recursors, and equalities $\Rightarrow$ OK to use recursor; equalities easily discharged by \lsti{reflexivity}
\item Instantiate definition of recursive functions (induction proofs) and equalities $\Rightarrow$ OK to use recursor; equalities easily discharged by \lsti{reflexivity}\\
      Code shared by code compiled for derived families
\item Include 
\end{itemize}
