...
code inherited from a base family need not be retranslated or rechecked.
...
For instance, consider field \lsti{env} in family \lsti{STLC}.
...
referenced through this \lsti{self}.
In particular, \lsti{env} is defined as \lsti{id -> option ty},
...
Further-bindable fields are translated to module types that export only types of the fields.
Non-further-bindable fields are translated to modules that export definitional equalities
on the fields.
...
%
\begin{itemize}[align=left,itemsep=5pt,labelsep=*,leftmargin=*]

\item
For \lsti{tm},
an inductive type \lsti{tm\_} is generated, with \lsti{tm\_}
instantiating the axiomatized \lsti{tm} type, and with
its constructors (\lsti{tm_unit\_} etc.) instantiating 
the axiomatized constructors (\lsti{tm_unit} etc.).
%
The partial recursor \lsti{tm_prect_STLC} is instantiated
%to \lsti{\lambda}\,\lsti{P, tm\__rect (\lambda}\,\lsti{t, option (P t))},
with the help of \lsti{tm\_rect}, the recursor of the generated \lsti{tm\_} type.
Proving the equalities on \lsti{tm_prect_STLC} is then trivial, by \lsti{reflexivity}.

\item
Similarly, \lsti{subst} is instantiated by applying the recursor \lsti{tm\_rect}
...
For example, the four case handlers for \lsti{subst} compiled for \lsti{STLC}
are reused by module \lsti{STLCFix}. So is the proof of \lsti{typesafe}.

Finally, the reference \lsti{STLCFix.typesafe} (where \lsti{STLCFix} is a family)
can simply be translated to \lsti{STLCFix.typesafe} (where \lsti{STLCFix} is a module),
