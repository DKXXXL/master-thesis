...
an \textit{overridable} family. Using the above example, we simply seal
\mintinline{Coq}{f : nat → nat} and \mintinline{Coq}{p : monotonic f}
...
``Sealed'' families can actually simulate interfaces in certain sense\YZ{I was actually thinking sealed families simulate sigma types, no?}\EDJreply{You are absolutely right. I should add a "can" here. I can use sealed family to simulate interface is by creating a sigma type, but making the implemntation of sigma type trivial (using place holder/using axiom as I specified below). Then once I want to instantiate with real implementation, I override the whole family -- as long as it has the same type as the sealed family, everything still works.}\YZreply{Can you explain this with an example? I recall the AbsInt example uses it?}---we can assign each field of the sealed family with \mintinline{Coq}{Axiom non_implement : forall {T : Type}, T.}, and later instantiate them with concrete implementation by overriding this family.\YZ{I don't understand what it means to "assign each field with an axiom".}\EDJreply{It really means ``place holder'' in general programming language (Python's NonImplement()), defer to future. Because Coq's Axiom can be used to instantiate anything.}
