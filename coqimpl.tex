...
\mintinline{Coq}{self_B.tt ≠ self_B.ff}.\YZ{Is it really impossible to prove constructor injectivity without partial recursors and with recursors only? My confusion after reading these paras is that I still don't know why partial recursors are needed.}
...
We call this \textit{propositional partial recursor}.\YZ{What is propositional about partial recursors? I suppose it has to do with the computational axioms?}
For example, the propositional partial recursor for \mintinline{Coq}{B.b} is\YZ{Mention that partial recursors can be automatically generated}
...
prec {R} r₁ r₂ .. rₙ (dᵢ aᵢ bᵢ) 
  ...
  \begin{minted}[escapeinside=@@]{Coq}
    forall c : C, linv (inj c) = some c@\YZ{Does 'linv' correpsond to 'reflect' above? If so, then the reader should be explicitly reminded of this fact rather than having to guess it.}@
  \end{minted}
\end{theorem}
...
being defined.\YZ{Please check if my wording is correct.}
...
finished defining a family, and thus a bit harder to handle.\YZ{'harder' in what way? I suppose you mean the "Fail Ltac" in Figure 5?}
...
the following fields of the family.\YZ{It reads like this MetaData wrapping could be automatically inserted, no?}
