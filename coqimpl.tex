...
are located in the family "handlers". This way, the reuse and the extension of the case handlers will be delegated by the inheritance of the family.
...
\textbf{Family compiled into module.} We support family by compiling
...
but our compilation will type check \mintinline{Coq}{c : a = 1 := eq_refl} in the context where we only know \mintinline{Coq}{a : nat} thus failed.  
...
families. Overriding field will always have these two fields "f, p".\YZ{Can an overriding, sealed family define new fields not in its parent?}\EDJreply{I don't find this a problem but I didn't support it in my plugin. }
...
To construct a recursor as in \ref{chg:extensible-inductive-type}, we need to create for each
...
However, we later still need the concrete eliminator "b" for doing type-checking (as in \ref{chg:extensible-inductive-type}), thus the information doesn't totally disappear but store elsewhere.  Here we show one exemplar extraction of the recursor to \mintinline{Coq}{Set} as module "b_rec_12". This concrete recursor will be used by our "FRecursor". For example, in \cref{fig:plugin-example2}, "FRecursor" command will feed Coq a module "v_33_34" for doing type-checking with specified handlers (e.g. "neg_handler") during compilation---what it does is basically
...
not expressive enough to really fulfill \ref{chg:extensible-inductive-type}. For
example, we cannot prove the constructors are injective, i.e.,
\mintinline{Coq}{self_B.tt ≠ self_B.ff} because from the perspective of Coq they are just two terms declared in the module type.\YZ{Is it really impossible to prove constructor injectivity without partial recursors and with recursors only? My confusion after reading these paras is that I still don't know why partial recursors are needed.}\EDJreply{The issue is exactly, at that point of programming, for extensibility, there is no access to the concrete recursor. Concrete recursors also cannot be postulated as part of the interface because it breaks extensibility.}
...
\begin{minted}{Coq}
  pdelim : ∀ (P : D -> Set), .., 
  (rᵢ : (∀ (x : Aᵢ) (w : Bᵢ x -> D), (∀ (b : Bᵢ x), option (P (w b))) -> option (P (dᵢ x w)))),  
  .., ∀ (d : D) -> option (P d)
\end{minted}
...
scenario can happen on other statement like \mintinline{Coq}{~ value
...
being defined.
% \YZ{Please check if my wording is correct.}\EDJreply{Yes. Resolved}
...
can solve the constraints.\YZ{This is a good one-liner that summarizes the intended usage scenarios of "Closing Fact". Should probably bring it out earlier.}
