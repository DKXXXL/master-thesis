...
% Explain the whole procedure using this pseudo-code example
Now that we know a family data structure is actually a list of compiled modules, we need to aggregate them into a complete compiled module. Thankfully, Coq's \textit{Include} command makes things easy and reasonably fast. One key feature of Coq's \textit{Include} is that, it can include not-completely instantiated functor (i.e. module with non-instantiated parameters) and it will try to find appropriate fields in the current surrounding module to instantiate those parameters. This feature can solve the fact that each defining field requires a ``self'' parameter.  
...
To simplify the implementation, we can consider each family as an inheritance and the standalone family are extending empty family. Thus we directly construct the data structure representing inheritance during interactive proving. An inheritance will fix the type for the input family and output family, and generally has three kinds of inheritance in OO language -- extend, override and inherit. Extending corresponds to adding new fields, overriding corresponds to modify existent fields and inheriting corresponds to directly taking fields from the parent. It is possible to consider inheritance as solely a data rather than a functor, and this actually provides us the ability to \text{mix-in} inheritance data. 




\textbf{Reasoning Requires Non-Overridable.}
Unlike general-purposed programming, Coq should be able to  \textit{reason} about each program. However, sometimes reasoning requires a specific program and overriding on them is unacceptable. For example, we may define a field $"add" : \mathbb{Z} \to \mathbb{Z} \to \mathbb{Z}$ as one field and intends to proving its commutativity in a following field $"comm"$. But to prove so, we need to know more than just a type interface of $"add"$ but also its \textbf{concrete definition}. Otherwise, in the future, once $"add"$ is overridden into a subtraction function then $"comm"$ will have problem to be  inherited. To achieve such exposure of definition, in meta-theory we only need to expose the information as propositional equality as part of the signature, and in our Coq plugin, we can achieve that even easier by exposing the whole definition directly into module type. Doing so we can have a better judgemental equality and we can avoid overriding because subtraction doesn't have the same definition as $add$. We require the users to decorate field as `Final' so that the plugin will proceed with this special treatment.
