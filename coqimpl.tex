...
But we do have to provide a way to construct a recursor, and execute type checking (or exhaustiveness checking). Thus we need to extract a elimination principle for future usage. Here we show one exemplar extraction of the recursor in \mintinline{Coq}{Set} as module "b_rec_12".
...
Apparently, the exposed module "b_3" in \cref*{fig:plugin-example2} is definitely not expressive enough. For example, we cannot prove the constructors are injective, i.e. \mintinline{Coq}{self_B.tt <> self_B.ff}. 
