...
However, "FRecursor" provides barely enough functionality for manipulating (extensible) inductive type, and it is far from satisfactory to be a legitimate programming interface---for example, the necessary tactic "inversion", "discriminate", "injection" is not applicable on (extensible) inductive type.
...
\textbf{Overridable pins Inductive Type and FRecursor}.

Also, when pins "FInductive" and "FRecursor", we can have the primitive coq-providing recursor and judgemental equality for computation ability for "FRecursor". 
