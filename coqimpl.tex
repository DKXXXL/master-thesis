...
\noindentparagraph{Trusted base.} Finally, we emphasize that our compilation can make sure the trusted code base is restricted to the Coq's kernel, as all the feature supported by our plugin has been compiled into Coq's surface syntax/Vernacular commands.
...
\begin{itemize}
  ...
  \item ...\YZ{We should mention the limitations mentioned in Review D and in our response. All these limitations can be split into two categories: (1) those that we believe can be fixed with more engineering effort, and (2) those that are more research-y.}
\end{itemize}
