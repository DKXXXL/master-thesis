...
Definition subst :=
  self__STLC.__internal_tm_rec __motiveTsubst
	self__STLC.subst_internal.tm_var self__STLC.subst_internal.tm_abs
    self__STLC.subst_internal.tm_app self__STLC.subst_internal.tm_unit.
End subst_128.
...
Include tm_true_197 STLC_bool.
Include tm_false_200 STLC_bool.
Include tm_if_203 STLC_bool.
(* The Reused Compilation result *)
Include tm_var_handle STLC_bool.
Include tm_abs_handle STLC_bool.
Include tm_app_handle STLC_bool.
Include tm_unit_handle STLC_bool.
End v_248.
...
module type "STLC_Interface". We cannot export
...
%several places...}
the eliminator of the inductive type "tm" to "STLC_Interface".
...
"tm_rec_type". This concrete eliminator will be used by our "FRecursor".
For example, in \cref{fig:plugin-example2}, "FRecursor" command will
feed Coq a module "do_tc" for doing type-checking with specified
...
but extended and modified) inductive type in that compiled module.\YZ{Where is this "tm" inductive type in figure 6?} Note
...
and exhaustiveness checking. The former is handled by family inheritance
...
have any computational information using this "FRecursor" until its
compilation into module, i.e. we cannot do computation with it during
...
% \textit{Generating Scheme for Propositional Computation Axiom for FRecursor}. 
...
Though partial recursor is provable by simply using "FRecursor", for convenience, we support using "FScheme" to automatic generate it. At the meantime, we implement a tactic "prec_discriminate" that when fed with the appropriate partial recursor (with computational axiom generated), it can discriminate an absurd equation between two different constructors of extensible inductive type.
\begin{minted}{Coq}
