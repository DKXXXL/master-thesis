...
\lsti!| .. | tm\_true | ..! & 2       & $["tm" : \cU, "tm_var" : id \to \El{"tm"} .. $                          & $["tm" : \cU,  ..$                         \\
                      &          & $"tm_unit" : \El{"tm"}]$      & $"tm_unit", "tm_true", .. ]$ \\ \cline{2-4} 
...
Our final target is to construct a term $\goodInh{\cdot}{?}{\lsig_5}{\lsig_5'}$ using the four constructors. There are two upcasting operations required $\goodTerm{A_3'}{\uparrow s_3}{A_3}$ and $\goodTerm{A_4'}{\uparrow s_4}{A_4}$ (omitting debruijn indices here) for checking the compatibility between $A_i$ and $A_i'$.
