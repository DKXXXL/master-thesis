...
This \textit{mixin} is implemented roughly by making inheritance judgement/data supporting \textit{weakening} and composition. Then, using notation from metatheory, given $\goodInh{\cdot}{i_1}{\sigma_1}{\sigma_2}$ and $\goodInh{\cdot}{i_2}{\sigma_1}{\sigma_3}$, we can weaken $i_2$ to $\goodInh{\cdot}{i_2'}{\sigma_2}{\sigma_3'}$ and the composition of $i_1$ and $i_2'$ is the resulting mixin---this implementation breaks commutativity and will need to solve (design) some compatibility solution: for example, when $i_1$ and $i_2$ are both extending with field of the same name, do we signal error or use $i_2$'s extension as considering it as an `override'? However, our current example of STLC only uses a subset of the mixin that is with a clear semantic (i.e. every sensible design of mixin should agree on).
...
exactly what we expect from "Closing Fact"---if directly working on inductive definition doesn't bring us benefit, then decompose the property (the constraint) with the concrete definition might be a good idea.
