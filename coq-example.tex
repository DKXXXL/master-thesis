...
To prove \mintinline{Coq}{forall s, (is_prog s) + (is_prog s -> False)}, we use strong induction on the length and the non-trivial part is at the inductive step, i.e. we need to show \mintinline{Coq}{(forall e, len e < len s -> (is_prog e) + (is_prog e -> False)) -> (is_prog s) + (is_prog s -> False)}. 
