In this section, we lay out the complete version of the syntax of \TT but in the style of \citeauthor{debruijn1964} indices and \emph{explicit substitutions}~\cite{substcalculus,abadi1989subst}:
substitutions~$\gm$ and their applications (e.g., in the form of \sub{T}{\gm}) are part
...
\noindentparagraph{Explicit Universe Level.} Our treatment of universe level mainly follows that of \citet{altkap2016, kaposi2019gluing}. Universe level is mainly to get around the size issue---we cannot refer to(and thus quantifiy over) a set of all set (or a universe of all type). So everytime we want to quantify over a universe of types, we have to specify ``how large'' that universe is, that is basically what universe level is measuring. 

% We first need the mathmatical setup to talk about universe level, following \citet{sterling2019algebraic}:
...
% For people familiar with Agda, we can consider $\Set{i}$ as the one in Agda. In other words, each $\Set{i}$ is (1). closed under dependent function type and dependent tuple type, (2). cumulative as $\Set{i} \in \Set{i+1}$ and $\Set{i} \subseteq \Set{i+1}$
