...
To simplify, $\SubstWeak{n}$ is a short hand for $\pi_1^n~"id"$ and $\var{n}$ is a short hand for $\pi_2~\pi_1^{n}$. Thus during meta-theoretic reasoning, we will only deal with $\pi_1$ and $\pi_2$.\YZ{$\pi_1$ and $\pi_2$ should be added to the syntax figure}

Finally, we have \ruleref{sub/dbj/shift} defined using \ruleref{sub/wk} and \ruleref{tm/var}. This rule applying substitution $\gm$ on the earlier part of the context. We usually omit $A$ in the $\gm^{\uparrow A}$ because it can be inferred from the context.\YZ{The shift rule has bugs: premise says n, conclusion says 1.}
...
%Moreover, the linkage transformer can be considered as a syntactic sugar (or
%``library functions'' in the main text) (we will show why at the end).
%Since it is a syntactic sugar, we will not consider about the linkage
%transformers during meta-theoretic reasoning.
...
as syntactic sugar via an inductive type with constructors\YZ{I reinterpreted what you wrote, but this sentence doesn't sound right to me. If linkage transformers are meta-level inductive types, aren't they just part of the syntax?}
$\InhExt{}{}$, $\InhOv{}{}$, $\InhInh{}$, $\InhNest{}{}$, and
the syntax $\inh{h}{\ell}$ and $\sub{h}{\gm}$ can be defined by induction on
this inductive type.
