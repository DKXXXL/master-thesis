...
Indeed, for MLTT, we only have syntactic term and their judgemental equality (conversion rules) among them. However, we still can have \emph{an interpreter} about our MLTT in our ambient meta-logic. For a programmer, this is just like writting a Haskell meta-circular interpreter~\cite{abelson1996structure} that interpret an abstract syntax tree into Haskell's data structure. For a logician, this is just interpreting the syntactic $\goodTerm{\cdot}{t}{\cB}$ to an element $t \in \{\true, \false \}$. For a formal semanticist, this can be considered as a naive form of denotational semantic. 
