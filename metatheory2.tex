...
% Deal with intuition
\noindentparagraph{The necessity of $\TyTkg{\cdot}$.} 
According to the equality rules of $\ruleref{tyeq/pk/add}$, for a fixed $\lsig$, $\TyTkg{\lsig}$ is structurally similar to $\TyLkg{\lsig}$ but we are using dependent tuple $\pair{\cdot}{\cdot}$ instead of $\LkgAdd{\cdot}{\cdot}$ to package the fields of a linkage. We have computed several examples in \cref{fig:stlc-linkage-typing}.
...
We illustrate with a small example using named convention, say given two inductive type $\wsig_1$ and $\wsig_2$ and a function $f$ maps from the first to the second type. Then we will have a linkage $\goodTerm{\cdot}{\ell}{\TyLkg{\lsig}}$. Say we want to add a new field into this linkage and abstracting both extensible inductive types, then in $\ruleref{l/add}$, the chosen $A$ must abstract both inductive type to $\cU$, resulting something like $\lsig^A$. The question now is whether $A$ is a linkage type $\TyLkg{\lsig^A}$ or a dependent tuple type $\TyTkg{\lsig^A}$; and for each case we will intuitively choose the following approach to do abstraction.

\begin{align*}
  & \text{doing abstraction} \\ 
...
However, the seemingly easier approach $\alpha$ is impossible because linkage $\TyLkg{\lsig^A}$ is uninhabited---say we have $\ell^A : \TyLkg{\lsig^A}$, then by projection we will have an impossible term $\goodTerm{T_A : \cU, T_B: \cU}{\ell^A.f}{\El{T_A} \to \El{T_B}}$ due to the nature of overridability of linkage. To make the inconsistency clearer, by double abstraction we will have an impossible function $g \coloneq \goodTerm{\cdot}{\nlam{T_A}{\nlam{T_B}{\ell^A.f}}}{\nTyPi{T_A}{\cU}{\nTyPi{T_B}{\cU}{\El{T_A} \to \El{T_B}}}} $, i.e. a function maps from arbitrary type to another arbitrary type . We can easily derive inconsistency setting applying $\napp{\napp{g}{\top}}{\bot} : \top \to \bot$. 
