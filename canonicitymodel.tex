...
There are actually more \emph{internal} type-theoretic construction to we need:
...
Basically the point is, for each well-typed type $\goodType{\Gm}{T}{j}$,  we have its denotation $\denotesCC{T} \in \GTy{j}{\denotesCC{\Gm}}$; and for each well-typed term $\goodTerm{\Gm}{t}{T}$, we have its denotation $\denotesCC{t} \in \GTm{\denotesCC{\Gm}}{\denotesCC{T}}$. Bascially the four sets are collecting the glued interpretation.
...
\newcommand{\CWsup}{\ensuremath{\mathit{Wsup}^C}}
\newcommand{\CWrec}{\ensuremath{\mathit{Wrec}^C}}
...
\begin{align*}
  "Inductive"~\CWmodel &: (\Glued{\wsig} \in \GWSig{i}{\denotesCC{\cdot}}{N}) \to \{ t : \goodTerm{\cdot}{t}{\El{\wcode{\wsig}}} \} \to \Set{i + 1} \quad "where" \\
    ...
    & \to \Glued{b} \in (\GTm{(\Glued{\cdot} \Glued{,} \sub{\wsigproj{j}{2}{\Glued{\wsig}}}{\SubstExt{\SubstId}{\Glued{a}}}) }{\sub{\El{\wcode{\wsig}}}{\SubstWeak{1}}}) \to \CWmodel~\Glued{\wsig}~\Wsup{j}{\wsig}{a}{b}
\end{align*}
...
\begin{align*}
  \CWrec &: (\Glued{\wsig} \in \GWSig{i}{\denotesCC{\cdot}}{N}) \to 
    ...
    & \to [j < N \to  \Glued{a} \in \GTm{\Glued{\cdot}}{\wsigproj{j}{1}{\Glued{\wsig}}} \\ 
    & \quad \quad \to 
    b \in (\GTm{({\cdot} {,} \sub{\wsigproj{j}{2}{{\wsig}}}{\SubstExt{\SubstId}{{a}}}) }{\sub{\El{\wcode{\wsig}}}{\SubstWeak{1}}}) \\ 
    & \quad \quad \to b^C : (\Glued{\gm} \in (\Glued{\cdot} \Glued{,} \sub{\wsigproj{j}{2}{\Glued{\wsig}}}{\SubstExt{\SubstId}{\Glued{a}}}) \to P~\sub{b}{\Gmodel{\Glued{\gm}}}) \\
    & \quad \quad \to P~(\Wsup{j}{\wsig}{a}{b}) ] \\
    & \to \goodTerm{\cdot}{t}{\El{\wcode{\wsig}}} \to \CWmodel~\Glued{\wsig}~t \to P~t \\
  \CWrec & \Glued{\wsig}~P~f~t~(\CWsup~\Glued{a}~\Glued{b}) = f~\Glued{a}~b~(\lambda )
\end{align*}

Note that in $\CWmodel$, the $\Glued{b}$ uses the definition of $\denotesC{\wcode{\wsig}}$, which after unfolding, will refers to $\CWmodel$ again and in strictly positive position. So it is a well-founded definition.
