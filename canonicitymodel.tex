Now we prove consistency and canonicity using a canoncity model.
We follow the reducibility argument of
\citet{kaposi2019gluing,coquand2018canonicity,sterling2019algebraic}
to construct this canonicity model.
%
...
Canonicity is a basic criterion that must be satisfied by a dependent type theory serving as
``a programming language or as a computational foundation for mathematics''.\footnote{See nlab explanation: \href{https://ncatlab.org/nlab/show/canonical+form}{Canonical Form}}
We can even argue that, if this theorem is proven in a computable meta\-logic, 
then by Curry–Howard Correspondence, the canonicity theorem provides a big-step
interpreter for closed term of boolean type.%\YZ{Can we say something about
...
we use $\Gmodel{\Glued{S}}$ to mean the latter of the two.\YZ{why not just $\denotesC{S}$?}
