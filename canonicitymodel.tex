...
then by Curry-Howard Correspondence, this theorem provides a big-step interpreter for closed term of boolean type.\YZ{Can we say something about strong/weak normalization?}\EDJreply{
  No. \\
  1. In the declarative system, we cannot talk about strong/weak normalization (because we don't have operational semantic, if you recall the definition of strong/weak normalization, it based on ). So any review says strong normalization to you please stop them. \\ 
  2. In declarative system, we only say ``normalization'' directly, which means, we construct a function that \\
    ...
    (apparently most people will make the syntax representation  without beta redex, however, interestingly, they may do eta expansion instead of eta reduction) \\ 
  3. So translation won't work because it compiles away the normal form of linkage (the $\mu^+$). This part requires direct proof \\
  4. Of course we can conjecture we have that normalization function \\
  5. Also we do have close term normalization, which is canonicity. Or more operationally speaking, we have termination (a runnable interpreter) for close term (since QIIT is runnable, the canoncity model ), \\
