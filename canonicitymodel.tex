...
There are actually more \emph{internal} type-theoretic construction to explore, and we need them to define the model for \TT as well. 
...
Basically the point is, for each well-typed type $\goodType{\Gm}{T}{j}$,  we have its denotation $\denotesCC{T} \in \GTy{j}{\denotesCC{\Gm}}$.
...
\end{align*}
together with the following inductively defined indexed set and its eliminator principle. These can be directly translated into Agda but we use a more conventional math notation
\begin{align*}
  "Inductive"~CW_p : (\Glued{\wsig} \in \GWSig{i}{\denotesCC{\cdot}}{N}) \to \{ t : \goodTerm{\cdot}{t}{\El{\wcode{\wsig}}} \} \to \Set{i + 1} \quad "where" \\
    "CWsup"_p : n < N \to 
\end{align*}
