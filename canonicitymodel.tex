...
\begin{align*}
  ...
  \text{we define }\TyLkg{}, \TyTkg{} & \text{ inductively on its input signature, via } \mathit{L},\Glued{\mathit{P}}  \\ 
...
\end{align*}
This is the first point our proof becomes cryptic because of this bullet notation. 
...
In all, the whole interpretation should be considered as:
\begin{enumerate}
  \item We first get $\denotesCC{\lkgproj{2}{\lkg}}$, the glued interpretation of the projection of the linkage 
  \item Then we apply it with two substitution,  $\Gsub{\_}{\GSubstExt{{\GSubstWeak{1}}}{s}}$ and $\Gsub{\_}{\GSubstExt{\Glued{("id")}}{ \Glued{\ell'} }}$
  \item Then we use glued interpretation of dependent tuple to combine it with $\Gpair{\Glued{\ell'}}{\_}$
\end{enumerate}

To reflect that, we use background colour to help parsing for the proof, and there is no meaning for the choice of colour.
...
\end{align*}
We note that, the interpretation of $\Wsup{i}{\wsig}{a}{b}$ is defined by $\CWsup$ (defined below) will take three elements, $i$, a glued $(\Gsub{\denotesCC{a}}{\Glued{\gm}})$ and a glued $(\Gsub{\denotesCC{b}}{{\Glued{\gm^\uparrow}}})$.  As we mentioned earlier, $\Glued{\gm}$ is the equivalent form of $(\gm, \gm')$.
...
\begin{align*}  
  ...
  & \Gmodel{\Gsub{\Gapp{\Gsub{\Gapp{\GRecproj{j}{\Gsub{\denotesCC{\lkg}}{\Glued{\gm}}}}}{\GSubstExt{\GSubstId}{\Glued{a}}}}}{\GSubstExt{\GSubstId}{\Glam{\Glued{\rho}}}}}(\epsilon)(\star) \\
  & \quad 
