...
% When programming in a direct style, before using FPOP, the STLC base module 
...
When programming in a direct style, before using FPOP, the STLC base module will take around 350 LOCs while using FPOP, the STLC base family takes about same amount of the code. 

However, without FPOP, STLC with one feature will require around 400 LOCs as the definition of STLC base is repeated. On the other hand, using FPOP, each feature as extended family only requires ranging from 100~250 LOCs. What's more, without FPOP, STLC with multiple features will require more and more while using FPOP's mixin feature, tens of LOCs is enough.
