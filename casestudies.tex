...
An exception is some seemingly trivial ``inversion lemmas''.
...
%
However, we observe that it is lighter-weight to use overriding (\cref{sec:override}):
...
inductive type in the proof and thus discharge the proof with \lsti{inversion}.
The plugin then automatically tries the same proof script in a derived
family to override the proof.

We note that the current plugin implementation does not support
