...
it adds new constructors to the inductive types ("FInductive") and
adds new cases to the recursive functions ("FRecursion") and induction proofs ("FInduction").
...
\noindentparagraph{Inductive types are extensible.}

In family "SLTC_fix", inductive type "tm" is formed by the four
constructors inherited from "STLC" and a fifth constructor "tm_fix".
...
%such as \lstinline[mathescape]{tm_app (tm_fix "f" (tm_abs "x" $\dadada$)) tm_unit},
its type \lstinline{tm -> tm -> tm} says "tm_app" can be applied to any
Gallina terms of type "tm",
which in family "SLTC_fix" include those constructed from "tm_fix".


\noindentparagraph{Induction remains exhaustive.}


\ref{chg:extensible-v-exhaustive}
...
% overridable, reminding us of \ref{chg:override-v-dependent}.  To
...
\textbf{Overridable Fields with dependency.}  Each field is by default non-overridable. By the discussion of \ref{chg:override-v-dependent}, we can simply reason and access the definition about the former fields in a later field, just like Field "a" and "b" in \cref{fig:plugin-example3} and we lose the ability to override. 
...
% We can also use "Family" and "SealFamily" to scope a (nested) family. The resulting family only support direct inheritance or "Override" (i.e. we can only override a sealed family as a whole together)---we will later explain this design decision and its usage, especially on resolving \ref{chg:override-v-dependent}. What's more, "Override" family can only apply to sealed family as well (not allowed on normal family).
...
% nested families, inductive types, and fields. "Final" is used to claim that the decorated field is non-overridable, and later we will show how it is used to resolve \ref{chg:override-v-dependent}. The fields inside a family
