...
don't want to lose too much reasoning power of Coq\YZ{By 'reasoning power of Coq', I suppose you mean the expressive power of Gallina?}
...
First and foremost, \textlabel{Req (0)}{langdesign-req0}: \textbf{we
desire incremental (modular) type checking}. Notice that, the modularity
here in Coq is a bit different from other languages, because Coq
supports \textit{interactive} theorem proving, so we actually need
\textit{statement-wise} incremental type checking, not only for avoiding
re\-compilation, but also for the immediate feedback and
type-checking for the Coq developer.
...
We use "Family" and "EndFamily" to scope.
Each field in a family is declared and defined as a "Field".\YZ{This sentence sounds a bit recursive}
