Integrating family polymorphism into an interactive theorem prover poses challenges
...
\noindentparagraph{Overriding vs.\ dependent typing.}
...
\noindentparagraph{Self reference vs.\ logical consistency.}
...
the implicit self parameterization is how they achieve late binding and thus extensibility.
However, self reference could easily lead to divergence.
Divergence is not a concern for the design of ordinary OO or
functional languages, but it means logical inconsistency---and
hence unsoundness---for a language aimed at logical reasoning.
A language design must ensure that the added extensibility does not
go rogue.

\noindentparagraph{User experience.}
...
For instance, it should be possible in our tool to
incrementally navigate through vernacular commands and construct proofs
using tactics with instant feedback on the proof state,
